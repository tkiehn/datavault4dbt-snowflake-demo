...
{{ log('Enter parser with parameter: ' ~ name ~ ' and value: ' ~ parameter , info=True) }}
    {% if datavault4dbt.is_something(yaml_metadata) %}
        ...
        {% elif required %}
            {{ log("[" ~ this ~ "] Error: yaml-metadata given, but required parameter '" ~ name ~ "' not defined in there or outside in the parameter. \n Description of parameter '" ~ name ~ "': \n" ~ documentation, true ) }}
    ...
    {% elif required %}
        ...
        {{ log("[" ~ this ~ "] Error: Required parameter '" ~ name ~ "' not defined. Define it either directly, or inside yaml-metadata. \n Description of parameter '" ~ name ~ "': \n" ~ documentation, true ) }}
    {% else %}
        {% set return_value = None %}
    {% endif %}
